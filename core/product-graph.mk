...
$(products_graph): $(this_makefile)
	@echo ${CL_GRN}"Product graph DOT:"${CL_RST}" $@ for $(PRIVATE_PRODUCTS_FILTER)"
...
$(OUT_DIR)/products/$(strip $(1)).txt: $(this_makefile)
	@echo ${CL_GRN}"Product debug info file:"${CL_RST}" $$@"
	$(hide) rm -f $$@
	$(hide) mkdir -p $$(dir $$@)
...
$(call product-debug-filename, $(p)): \
			...
			$(this_makefile)
	@echo ${CL_GRN}"Product debug html file:"${CL_RST}" $$@"
	$(hide) mkdir -p $$(dir $$@)
...
$(products_pdf): $(products_graph)
	@echo ${CL_GRN}"Product graph PDF:"${CL_RST}" $@"
	dot -Tpdf -Nshape=box -o $@ $<

$(products_svg): $(products_graph) $(product_debug_files)
	@echo ${CL_GRN}"Product graph SVG:"${CL_RST}" $@"
